...
#
# Copyright 2013 Martín Ferrari <tincho@tincho.org>
#
...
# The following options can be overridden on a per-feed basis:
...
class Feed(object):
    DB_FIELDS = ('etag', 'modified', 'seen', 'disabled')
    ...
    def __init__(self, feed_config, dbdata=None):
        ...
        self.subtitle = None
        self.link = None
        self.author_name = self.author_href = self.author_email = None
        if dbdata is None:
            dbdata = {}
        # Status fields, saved in the FeedDB.
        for key in self.DB_FIELDS:
            setattr(self, key, dbdata.get(key, None))
        if not self.seen:
            self.seen = dict()
    ...
    def fetch(self, global_config):
        ...
        self.title = feedobj.feed.get('title', None)
        self.subtitle = feedobj.feed.get('subtitle', None)
        self.link = feedobj.feed.get('link', None)

        if 'author_detail' in feedobj.feed:
            self.author_name = feedobj.feed.author_detail.get('name', None)
            self.author_href = feedobj.feed.author_detail.get('href', None)
            self.author_email = feedobj.feed.author_detail.get('email', None)
        ...
        for entry in reversed(feedobj.entries):
            parsed = FeedEntry(entry)
            if (parsed.id in self.seen and (
                not global_config.repost_changes or
                seen[parsed.id] == parsed.content_hash)
            ):
                continue
            self.seen[parsed.id] = parsed.content_hash
...
class FeedConfig(object):
    ...
    def __init__(self, feed_name, feed_url,
                 ...
                 use_html=None, repost_changes=None,
...
class FeedEntry(object):
    def __init__(self, raw_entry):
        if 'title_detail' in raw_entry:
            self.text_title, self.html_title = get_text_and_html(
                [raw_entry.title_detail])
        else:
            self.text_title, self.html_title = None, None

        if 'author_detail' in raw_entry:
            self.author_name = raw_entry.author_detail.get('name', None)
            self.author_href = raw_entry.author_detail.get('href', None)
            self.author_email = raw_entry.author_detail.get('email', None)
        else:
            self.author_name = self.author_href = self.author_email = None

        raw_content = raw_entry.get('content', [])
        if 'summary_detail' in raw_entry:
            raw_content.append(raw_entry.summary_detail)

        # Serialise and hash the content.
        content_hash = hashlib.sha256(yaml.dump(raw_content)).hexdigest()

        self.id = raw_entry.get('id', content_hash)
        self.content_hash = content_hash

        self.text_content, self.html_content = get_text_and_html(raw_content)
        self.link = raw_entry.get('link', None)
        self.comments = raw_entry.get('comments', None)
        self.updated = raw_entry.get('updated_parsed', None)
        self.published = raw_entry.get('published_parsed', None)
        self.tags = raw_entry.get('tags', None)
...
def parse_cmdline(argv):
    ...
    for cmd in ('add', 'set'):
        ...
        # FIXME: usar none.
        subparser[cmd].add_argument(
            '--use_html', action='store_true',
            help='Send HTML or plain text emails (overrides config).')
        subparser[cmd].add_argument(
            '--repost_changes', action='store_true',
            help='Send a new email when a post changes.')
        subparser[cmd].add_argument(
            '--headers', metavar="HEADER:CONTENT", action='append',
            help='Extra header to add to the email (can be repeated).')
    ...
    if 'headers' in result and result.headers:
        header_re = re.compile(r'^([\033-\071\073-\176]+):\s*([^\r\n]+)$')
        for header in result.headers:
            if header_re.search(header):
                raise ConfigError('Invalid header specified.')
...
def get_text_and_html(detail):
    """Function to handle the various *_detail fields from feedparser.

    It will scan the components and return the first html and plain text
    components it can find; if either is not present, it will convert HTML to
    text or viceversa.

    Args:
        detail (list): List of dictionaries taken from a _detail field.

    Returns:
        (tuple) A tuple of strings (html_content, text_content).
    """
    html_contents = []
    text_contents = []
    for item in detail:
    ...
    for feed in feeds:
        ...
        for entry in new_entries:
            if use_html:
                ...
                msg.attach(email.mime.text.MIMEText(entry.text_content,
                                                    'plain', 'utf-8'))
                msg.attach(email.mime.text.MIMEText(entry.html_content,
                                                    'html', 'utf-8'))
            else:
                msg = email.mime.text.MIMEText(entry.text_content,
                                               'plain', 'utf-8')

            msg['Subject'] = '%s: %s' % (feed.title, entry.text_title)

            default_from = email.utils.parseaddr(config.default_from)
            author_name = (entry.author_name or feed.author_name or
                           default_from[0])
            author_email = (entry.author_email or feed.author_email or
                            default_from[1])

            if feed.feed_config.force_from:
                msg['From'] = feed.feed_config.force_from
            elif config.use_author:
                msg['From'] = email.utils.formataddr(
                    (author_name, author_email))
            else:
                msg['From'] = config.default_from

            if feed.feed_config.force_to:
                msg['To'] = feed.feed_config.force_to
            else:
...
def do_add(config, args):
    ...
    feedconf = FeedConfig(feed_name=name, feed_url=args.feed_url,
                          ...
                          repost_changes=args.repost_changes,
